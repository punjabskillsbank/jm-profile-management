...
public class FreelancerProfileController {
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "201", description = "Freelancer profile created successfully"),
    ...
    public ResponseEntity<FreelancerDTO> createProfile(@Valid @RequestBody FreelancerDTO freelancerDTO) {
        FreelancerDTO savedProfile = freelancerProfileService.createFreelancerProfile(freelancerDTO);
        return ResponseEntity.status(HttpStatus.CREATED).body(savedProfile);
    }

    @Operation(summary = "Get freelancer profile by ID", description = "Retrieve freelancer profile information using freelancer ID (mapped to user_id column in database)")
    @ApiResponses(value = {
